...
The LSTM hypoglycemia classifier lives under `ml/` so model code and binary artifacts are kept together and the risk API can prioritize short-horizon hypoglycemia detection:

```text
ml/lstm_hypoglycemia_classifier.py       # training/evaluation pipeline
ml/predict_lstm_hypoglycemia.py          # API inference entrypoint
ml/models/lstm_hypoglycemia_classifier.h5
ml/models/optimal_threshold.npy          # optional; defaults to 0.50 when missing
ml/models/lstm_feature_scaler.pkl        # generated by training; used when present
```

Install the Python inference dependencies before enabling the LSTM in a runtime environment:

```bash
pip install -r ml/requirements.txt
```

Train or refresh the classifier from the repository root with:

```bash
npm run ml:hypo
```

The script resolves paths from its own location, so it can load the model from `ml/models/` and detect either the original UoM folder layout or the demo CSVs in `data/raw/demo/`. During training it now also saves the feature scaler required by live inference.

At runtime, `/app-api/risk` builds the latest 48 five-minute timesteps from CGM, bolus, carb, and step signals, calls `ml/predict_lstm_hypoglycemia.py`, and exposes the result as `hypoglycemia_lstm`. If the LSTM alert is positive, the 24h risk is lifted by the LSTM probability and `needs_human_review` is set. Optional overrides:

```env
HYPO_LSTM_THRESHOLD=0.5
HYPO_LSTM_MODEL_PATH=ml/models/lstm_hypoglycemia_classifier.h5
HYPO_LSTM_THRESHOLD_PATH=ml/models/optimal_threshold.npy
HYPO_LSTM_SCALER_PATH=ml/models/lstm_feature_scaler.pkl
PYTHON_BIN=python3
```
