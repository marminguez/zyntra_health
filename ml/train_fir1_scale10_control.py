...
def main():
 ...
 print('=== FIR-1 SCALE-10 CONTROL — FULL CONVERGENCE ===');print(f'train windows: {sum(n for _,n in it):,} | validation: {len(y):,} | epochs max: {a.epochs}');print('EarlyStopping patience=4 | ReduceLROnPlateau patience=2 | future CGM: False')
 ...
 base={'mard':14.5988565,'rmse':30.88723775,'direction':.6916525};now={'mard':float(df.mard.mean()),'rmse':float(df.rmse.mean()),'direction':float(df.direction_accuracy.mean())};relm=100*(now['mard']/base['mard']-1);relr=100*(now['rmse']/base['rmse']-1);dpp=100*(now['direction']-base['direction']);best_epoch=int(np.argmin(hist.history['val_loss'])+1)
 print('\nVALIDATION');print(df.to_string(index=False));print('\n4H VS SCALE10');print(f"MARD {base['mard']:.6f} -> {now['mard']:.6f} ({relm:+.2f}% relative)");print(f"RMSE {base['rmse']:.6f} -> {now['rmse']:.6f} ({relr:+.2f}% relative)");print(f"Direction {100*base['direction']:.4f}% -> {100*now['direction']:.4f}% ({dpp:+.3f} pp)");print(f'Best val_loss epoch: {best_epoch} | epochs run: {len(hist.history["loss"])}')
 (out/'report.json').write_text(json.dumps({'experiment':'FIR-1 Scale10 full convergence control','epochs_max':a.epochs,'epochs_run':len(hist.history['loss']),'best_val_loss_epoch':best_epoch,'early_stopping_patience':4,'reduce_lr_patience':2,'metrics':now,'relative_mard_pct':relm,'relative_rmse_pct':relr,'direction_pp':dpp},indent=2))
