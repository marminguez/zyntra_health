...
Same data, seed, max epochs, future summaries and 50/50 fusion as the volatility
screen; the only removed input is the explicit 8-feature CGM volatility block.
"""
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--data-dir',default='ml/data/v15_4_scale10');ap.add_argument('--v14-1-dir',default='ml/results/v14_1');ap.add_argument('--outdir',default='ml/results/fir1_scale10_control');ap.add_argument('--epochs',type=int,default=5);ap.add_argument('--batch-size',type=int,default=64);a=ap.parse_args();random.seed(SEED);np.random.seed(SEED);tf.random.set_seed(SEED)
 ...
 sv=build_future_intervention_summary(fr,basal_mean=float(mean[5]),basal_std=float(std[5]));xv=((x-mean)/std).astype(np.float32);fv=nf(fr,mean,std);tar=tuple(y[:,j] for j in range(4))+tuple((y-cur[:,None])[:,j] for j in range(4));seq=Seq(it,mean,std,a.batch_size);m=build_fir1_forecaster(x.shape[1],x.shape[2]);cb=[tf.keras.callbacks.EarlyStopping(monitor='val_loss',patience=2,restore_best_weights=True),tf.keras.callbacks.ModelCheckpoint(str(out/'best.weights.h5'),monitor='val_loss',save_best_only=True,save_weights_only=True)]
 print('=== FIR-1 SCALE-10 CONTROL ===');print(f'train windows: {sum(n for _,n in it):,} | validation: {len(y):,} | epochs max: {a.epochs}');print('controlled difference vs volatility screen: explicit volatility block removed | future CGM: False')
 m.fit(seq,validation_data=([xv,fv,sv],tar),epochs=a.epochs,callbacks=cb,verbose=2);m.load_weights(out/'best.weights.h5');raw=m.predict([xv,fv,sv],batch_size=a.batch_size,verbose=1);ab=np.column_stack([raw[i].ravel() for i in range(4)]);dr=cur[:,None]+np.column_stack([raw[i+4].ravel() for i in range(4)]);pred=.5*ab+.5*dr;df=evalm(y,pred,cur);df.to_csv(out/'validation_metrics.csv',index=False);np.savez_compressed(out/'validation_predictions.npz',y_true=y,current_glucose=cur,y_pred=pred)
 base={'mard':14.5988565,'rmse':30.88723775,'direction':.6916525};now={'mard':float(df.mard.mean()),'rmse':float(df.rmse.mean()),'direction':float(df.direction_accuracy.mean())};relm=100*(now['mard']/base['mard']-1);relr=100*(now['rmse']/base['rmse']-1);dpp=100*(now['direction']-base['direction'])
 print('\nVALIDATION');print(df.to_string(index=False));print('\n4H VS SCALE10');print(f"MARD {base['mard']:.6f} -> {now['mard']:.6f} ({relm:+.2f}% relative)");print(f"RMSE {base['rmse']:.6f} -> {now['rmse']:.6f} ({relr:+.2f}% relative)");print(f"Direction {100*base['direction']:.4f}% -> {100*now['direction']:.4f}% ({dpp:+.3f} pp)");print('\nUse this run as the controlled comparator for fir_volatility_scale10; do not promote from this control alone.')
 (out/'report.json').write_text(json.dumps({'experiment':'FIR-1 Scale10 controlled baseline','purpose':'controlled comparator for Volatility-Aware FIR Scale10','controlled_difference':'explicit volatility block absent','baseline':'V15.4 Scale10','metrics':now,'relative_mard_pct':relm,'relative_rmse_pct':relr,'direction_pp':dpp},indent=2))
