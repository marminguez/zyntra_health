"""Train V15.3 from V15 master: future basal + bolus + carbs, excluding aggregate insulin."""
...
from ml.forecasting.model_v15_3 import build_v15_3_forecaster,HORIZONS
SEED=42
# master: insulin,im,basal,bm,bolus,bom,carbs,cm
CHANNELS=(2,3,4,5,6,7)
def load(root,split):
 xs,fs,ys=[],[],[]
 for p in sorted((root/split).glob('*.npz')):
  with np.load(p,allow_pickle=False) as z:
   x=z['x'].astype(np.float32);f=z['future_known'].astype(np.float32);y=z['y'].astype(np.float32)
  if f.shape[-1]<8:raise ValueError(f'{p}: expected v15_master channels, got {f.shape}')
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--data-dir',default='ml/data/v15_master');ap.add_argument('--v14-1-dir',default='ml/results/v14_1');ap.add_argument('--outdir',default='ml/results/v15_3');ap.add_argument('--epochs',type=int,default=20);ap.add_argument('--batch-size',type=int,default=64);a=ap.parse_args();random.seed(SEED);np.random.seed(SEED);tf.random.set_seed(SEED)
 ...
 ma.to_csv(out/'absolute_head_metrics.csv',index=False);mr.to_csv(out/'delta_reconstructed_metrics.csv',index=False);mf.to_csv(out/'v15_3_validation_metrics.csv',index=False);pd.DataFrame(hist.history).to_csv(out/'training_history.csv',index=False);np.savez_compressed(out/'validation_predictions.npz',y_true=yv,current_glucose=cv,absolute_head=pa,delta_head=dd,delta_reconstructed=pr,y_pred=pf)
 report={'version':'v15.3','data_source':'v15_master exact frozen V15.1 anchors','resampled':False,'hypothesis':'decomposed future basal and bolus outperform aggregate insulin when combined with carbs','controlled_change_vs_v15_2':'replace aggregate insulin with basal + bolus; carbs unchanged','future_features':['basal','basal_missing','bolus','bolus_missing','carbs','carbs_missing'],'future_cgm_input':False,'fusion':'fixed 0.5 absolute + 0.5 reconstructed delta','normalization':'V14.1 train-only statistics','seed':SEED,'train_windows':int(len(xt)),'validation_windows':int(len(xv)),'test_parquet_used':False,'live_targets_used':False,'clinical_status':'conditional challenge/research model; not causal clinical deployment'};(out/'v15_3_report.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
 print('\nV15.3 VALIDATION METRICS — FUTURE BASAL + BOLUS + CARBS');print(mf.to_string(index=False));print('\nComponent RMSEs');print(pd.DataFrame({'horizon':HORIZONS,'absolute_rmse':ma.rmse,'delta_reconstructed_rmse':mr.rmse,'hybrid_rmse':mf.rmse}).to_string(index=False));print(f'\nArtifacts written to {out}')
