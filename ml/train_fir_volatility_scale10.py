...
def main():
 ...
 print('=== VOLATILITY-AWARE FIR SCALE-10 — FULL CONVERGENCE / RESUMABLE ===');print(f'train windows: {sum(n for _,n in it):,} | validation: {len(y):,} | explicit volatility features: 8 | epochs max: {a.epochs}');print('BackupAndRestore ON | EarlyStopping patience=4 | ReduceLROnPlateau patience=2 | future CGM: False')
 ...
 base={'mard':14.5988565,'rmse':30.88723775,'direction':.6916525};now={'mard':float(df.mard.mean()),'rmse':float(df.rmse.mean()),'direction':float(df.direction_accuracy.mean())};relm=100*(now['mard']/base['mard']-1);relr=100*(now['rmse']/base['rmse']-1);dpp=100*(now['direction']-base['direction']);best_epoch=int(np.argmin(hist.history['val_loss'])+1) if hist.history.get('val_loss') else None;go=(relm<=-2 or relr<=-2) and dpp>=-.25
 print('\nVALIDATION');print(df.to_string(index=False));print('\n4H VS SCALE10');print(f"MARD {base['mard']:.6f} -> {now['mard']:.6f} ({relm:+.2f}% relative)");print(f"RMSE {base['rmse']:.6f} -> {now['rmse']:.6f} ({relr:+.2f}% relative)");print(f"Direction {100*base['direction']:.4f}% -> {100*now['direction']:.4f}% ({dpp:+.3f} pp)");print(f'Best val_loss epoch in this process: {best_epoch} | epochs run in this process: {len(hist.history.get("loss",[]))}');print(f"VOLATILITY-AWARE FIR VS SCALE10: {'GO' if go else 'NO-GO'}")
 (out/'report.json').write_text(json.dumps({'experiment':'Volatility-Aware FIR Scale10 full convergence','epochs_max':a.epochs,'best_val_loss_epoch_this_process':best_epoch,'early_stopping_patience':4,'reduce_lr_patience':2,'resumable':True,'features':['std15','range15','std30','range30','std60','range60','std120','range120'],'metrics':now,'relative_mard_pct':relm,'relative_rmse_pct':relr,'direction_pp':dpp,'go_vs_scale10':bool(go)},indent=2))
