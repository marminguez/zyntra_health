"""Scale-10 full-convergence screen: FIR-1 + eight explicit CGM volatility features."""
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--data-dir',default='ml/data/v15_4_scale10');ap.add_argument('--v14-1-dir',default='ml/results/v14_1');ap.add_argument('--outdir',default='ml/results/fir_volatility_scale10_20ep');ap.add_argument('--epochs',type=int,default=20);ap.add_argument('--batch-size',type=int,default=64);a=ap.parse_args();random.seed(SEED);np.random.seed(SEED);tf.random.set_seed(SEED)
 ...
 print('=== VOLATILITY-AWARE FIR SCALE-10 — FULL CONVERGENCE ===');print(f'train windows: {sum(n for _,n in it):,} | validation: {len(y):,} | explicit volatility features: 8 | epochs max: {a.epochs}');print('EarlyStopping patience=4 | ReduceLROnPlateau patience=2 | future CGM: False')
 hist=m.fit(seq,validation_data=([xv,fv,sv,vv],tar),epochs=a.epochs,callbacks=cb,verbose=2);m.load_weights(out/'best.weights.h5');raw=m.predict([xv,fv,sv,vv],batch_size=a.batch_size,verbose=1);ab=np.column_stack([raw[i].ravel() for i in range(4)]);dr=cur[:,None]+np.column_stack([raw[i+4].ravel() for i in range(4)]);pred=.5*ab+.5*dr;df=evalm(y,pred,cur);df.to_csv(out/'validation_metrics.csv',index=False);np.savez_compressed(out/'validation_predictions.npz',y_true=y,current_glucose=cur,y_pred=pred)
 base={'mard':14.5988565,'rmse':30.88723775,'direction':.6916525};now={'mard':float(df.mard.mean()),'rmse':float(df.rmse.mean()),'direction':float(df.direction_accuracy.mean())};relm=100*(now['mard']/base['mard']-1);relr=100*(now['rmse']/base['rmse']-1);dpp=100*(now['direction']-base['direction']);best_epoch=int(np.argmin(hist.history['val_loss'])+1);go=(relm<=-2 or relr<=-2) and dpp>=-.25
 print('\nVALIDATION');print(df.to_string(index=False));print('\n4H VS SCALE10');print(f"MARD {base['mard']:.6f} -> {now['mard']:.6f} ({relm:+.2f}% relative)");print(f"RMSE {base['rmse']:.6f} -> {now['rmse']:.6f} ({relr:+.2f}% relative)");print(f"Direction {100*base['direction']:.4f}% -> {100*now['direction']:.4f}% ({dpp:+.3f} pp)");print(f'Best val_loss epoch: {best_epoch} | epochs run: {len(hist.history["loss"])}');print(f"VOLATILITY-AWARE FIR VS SCALE10: {'GO' if go else 'NO-GO'}")
 (out/'report.json').write_text(json.dumps({'experiment':'Volatility-Aware FIR Scale10 full convergence','epochs_max':a.epochs,'epochs_run':len(hist.history['loss']),'best_val_loss_epoch':best_epoch,'early_stopping_patience':4,'reduce_lr_patience':2,'features':['std15','range15','std30','range30','std60','range60','std120','range120'],'metrics':now,'relative_mard_pct':relm,'relative_rmse_pct':relr,'direction_pp':dpp,'go_vs_scale10':bool(go)},indent=2))
