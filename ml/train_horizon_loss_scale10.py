"""Fast Scale-10 screen for the competition-aligned V15.4 loss.

Controlled experiment versus V15.4 Scale-10:
- same Scale-10 train/validation windows
- same V14.1 frozen normalization
- same V15.4 architecture and future-known inputs
- same fixed 50/50 absolute + reconstructed-delta inference
- only training objective/loss weights change
"""
...
from forecasting.model_v15_4_competition_loss import (
    HORIZONS,
    PREFIX_STEPS,
    build_v15_4_competition_loss_forecaster,
)
...
def main():
    ...
    ap.add_argument("--outdir", default="ml/results/horizon_loss_scale10")
    ...
    train_seq = ShardSequence(manifest, mean, std, args.batch_size)
    model = build_v15_4_competition_loss_forecaster(xv.shape[1], xv.shape[2])
    ...
    print("=== HORIZON LOSS SCALE-10 FAST SCREEN ===")
    ...
    print("architecture: exact V15.4")
    print("absolute loss: 75% robust absolute + 25% robust relative")
    print("horizon weights: 30=1.0, 60=1.1, 90=1.2, 120=1.3")
    ...
    mf.to_csv(out / "horizon_loss_scale10_validation_metrics.csv", index=False)
    np.savez_compressed(
    ...
    rel_mard = (mean_mard / BASELINE_MARD - 1.0) * 100.0
    rel_rmse = (mean_rmse / BASELINE_RMSE - 1.0) * 100.0
    dir_pp = (mean_direction - BASELINE_DIRECTION) * 100.0

    # Frozen fast-screen gate: material MARD gain or direction gain, without
    # material RMSE regression. Scale-50 is forbidden unless this gate passes.
    go = ((rel_mard <= -2.0) or (dir_pp >= 0.5)) and (rel_rmse <= 1.0)

    report = {
        "experiment": "horizon-loss-scale10-fast-screen",
        "scientific_change": "competition-aligned absolute loss + horizon loss weights",
        "architecture": "exact V15.4",
