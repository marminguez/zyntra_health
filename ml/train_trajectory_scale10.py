...
Only the training protocol changes: max 20 epochs, patience 4/2 and resumable backup.
"""
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--data-dir',default='ml/data/v15_4_scale10');ap.add_argument('--v14-1-dir',default='ml/results/v14_1');ap.add_argument('--outdir',default='ml/results/trajectory_scale10_20ep');ap.add_argument('--epochs',type=int,default=20);ap.add_argument('--batch-size',type=int,default=64);a=ap.parse_args()
 ...
 cb=[tf.keras.callbacks.BackupAndRestore(backup_dir=str(out/'training_backup'),save_freq='epoch',delete_checkpoint=False),tf.keras.callbacks.EarlyStopping(monitor='val_loss',patience=4,restore_best_weights=True),tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss',factor=.5,patience=2,min_lr=1e-5),tf.keras.callbacks.ModelCheckpoint(str(out/'best.weights.h5'),monitor='val_loss',save_best_only=True,save_weights_only=True),tf.keras.callbacks.CSVLogger(str(out/'training_history.csv'),append=True)]
 print('=== TRAJECTORY SCALE-10 — FULL CONVERGENCE / RESUMABLE ===');print(f'train windows: {ntrain:,} | validation: {len(y):,} | epochs max: {a.epochs}');print('decoder: 24 x 5 min | reads: 6/12/18/24 | future CGM: False');print('BackupAndRestore ON | EarlyStopping patience=4 | ReduceLROnPlateau patience=2')
 ...
 report={'experiment':'trajectory-scale10-full-convergence','scientific_change':'single causal recurrent future trajectory decoder with reads at 30/60/90/120 min','training_protocol':'20ep max; ES4; ReduceLR2; resumable','read_steps':READ_STEPS,'seed':SEED,'train_windows':int(ntrain),'validation_windows':int(len(y)),'epochs_max':a.epochs,'best_val_loss_epoch_this_process':bestep,'future_cgm_input':False,**means};(out/'trajectory_scale10_report.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
