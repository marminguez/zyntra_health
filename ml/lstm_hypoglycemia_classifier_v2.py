...
def split_dataset(df):
    ...
    if len(patient_ids) == 2:
        ...
        return train_df, val_df, test_df, {
            "strategy":"hybrid_two_patient","train_ids":[patient_ids[0]],
            "val_ids":[patient_ids[0]],"test_ids":[patient_ids[1]],
        }
    train_df, val_df, test_df = chronological_split(df)
    return train_df, val_df, test_df, {
        "strategy":"chronological_single_patient","train_ids":patient_ids,
        "val_ids":patient_ids,"test_ids":patient_ids,
...
def frames_to_sequences(frame, include_metadata=False):
    """Create windows per patient and optionally preserve target-row clinical metadata."""
    xs, ys, metas = [], [], []
    for pid in sorted(frame["p_id"].unique()):
        ...
        if include_metadata:
            ...
            metas.append(pd.DataFrame({
                "p_id": target_rows["p_id"].values,
                "timestamp": target_rows.index,
                # glucose is scaled in frame; glucose_future target remains original mg/dL.
                # Reconstruct current glucose from target relation is not safe, so caller supplies
                # an unscaled metadata frame aligned by patient/timestamp below.
            }))
    if not xs:
        empty_x = np.empty((0, LOOKBACK, 0))
    ...
    if include_metadata:
        return x_all, y_all, pd.concat(metas, ignore_index=True)
    return x_all, y_all


def attach_unscaled_metadata(sequence_meta, unscaled_test_df):
    """Attach original current glucose values after sequence creation without leaking into training."""
    ...
    sequence_meta = sequence_meta.copy()
    sequence_meta["timestamp"] = pd.to_datetime(sequence_meta["timestamp"])
    merged = sequence_meta.merge(source[["p_id", "timestamp", "glucose"]], on=["p_id", "timestamp"], how="left")
    if merged["glucose"].isna().any():
        raise RuntimeError("Could not align original glucose metadata with test sequences.")
    return merged
...
def calculate_metrics(model, x_test, y_test, threshold):
    ...
    return {
        ...
        "test_samples":int(len(y_test)),"test_positive_samples":int(y_test.sum()),
    }, probabilities
...
def main():
    ...
    x_test, y_test, test_meta = frames_to_sequences(test_df, include_metadata=True)
    ...
    model.compile(optimizer="adam", loss="binary_crossentropy", metrics=[
        tf.keras.metrics.Precision(name="precision"), tf.keras.metrics.Recall(name="recall"), tf.keras.metrics.AUC(name="auc")])
    model.fit(x_train, y_train, validation_data=(x_val,y_val), class_weight=class_weights,
              epochs=50, batch_size=64,
              callbacks=[EarlyStopping(monitor="val_auc",patience=15,restore_best_weights=True,mode="max")], verbose=1)

    threshold = float(find_optimal_threshold(model, x_val, y_val, beta=BETA))
    sample_metrics, probabilities = calculate_metrics(model, x_test, y_test, threshold)
    ...
    report = {
        ...
        "prediction_horizon_minutes":30,"lookback_minutes":LOOKBACK*5,
        "threshold":threshold,"threshold_metric":f"F{BETA}","split":split_info,
        ...
        "limitations":[
            ...
            "Event metrics require validation on a multi-patient dataset before clinical interpretation.",
        ],
    }
