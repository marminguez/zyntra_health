"""Train V15.2: V15.1 plus known future carbohydrate information."""
...
def load(root,split):
    ...
    for p in sorted((root/split).glob('*.npz')):
        with np.load(p,allow_pickle=False) as z:xs.append(z['x'].astype(np.float32));fs.append(z['future_known'].astype(np.float32));ys.append(z['y'].astype(np.float32))
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--data-dir',default='ml/data/v15_2');ap.add_argument('--v14-1-dir',default='ml/results/v14_1');ap.add_argument('--outdir',default='ml/results/v15_2');ap.add_argument('--epochs',type=int,default=20);ap.add_argument('--batch-size',type=int,default=64);a=ap.parse_args()
    ...
    # Reuse V14.1 train-only normalization for insulin (index 9) and carbs (index 11).
    for arr in (ft,fv):
        ip=arr[:,:,1]<.5;cp=arr[:,:,3]<.5;arr[:,:,0]=np.where(ip,(arr[:,:,0]-float(mean[9]))/float(std[9]),0.);arr[:,:,2]=np.where(cp,(arr[:,:,2]-float(mean[11]))/float(std[11]),0.)
